...
void scambiaCarte(char * [NPLAYER][ROW][CLM], int[NPLAYER][ROW][CLM], int, int, int);
void analizzaMano(char * [NPLAYER][ROW][CLM], int[NPLAYER][ROW][CLM], int, int);
...
void controlloMano(char *manoGiocatore[NPLAYER][ROW][CLM], int sortOrder[NPLAYER][ROW][CLM])
{
    ...
    //analizzaMano(manoGiocatore, sortOrder, 0, combinazioneMaggiore[0][1]);
    ...
    */
    ...
    printf("\nIl vincitore e' il giocatore nr %d", vincitore[0][1] + 1);

    return;
}

void scambiaCarte(int sortOrder[NPLAYER][ROW][CLM], int mazzoCarte[][2])
{

    for(int n = from; n < 4; n++)
    {
        int tmp;
        tmp = face[mazzoCarte[n][1]];
...
void analizzaMano(char *manoGiocatori[NPLAYER][ROW][CLM], int sortOrder[NPLAYER][ROW][CLM], int idPlayer, int mano)
{

    /* 
            poker, colore, scala, tris, doppiaCoppia, coppia, cartaAlta
 */
