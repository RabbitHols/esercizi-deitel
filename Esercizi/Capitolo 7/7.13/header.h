...
/*
    In caso di pareggio uso questa funzione per ricavarmi il giocatore con la carta 
    più alta
 */

    int cartaMaggioreMano[NPLAYER]; 

/* 
    Assegno alla mano del giocatore le carte e le coordinate delle carte estratte 
    dal mazzo.
 */

    int carteDaDistribuire = ROW * NPLAYER;

    for(int tab = 0; tab < NPLAYER; tab++)
    {
        for(int riga = 0; riga < carteDaDistribuire; riga++)
        {
            manoGiocatore[tab][riga][1] = wFace[hand[riga][1]];
            manoGiocatore[tab][riga][1] = wSuit[hand[riga][0]];
        }
    }

    
    /*  TODO: Creare una funzione sort con un array 3D
        che riordini in un unico sort i mazzi dei giocatori.
        l'idea è di farlo tramite un for non lo so da vedere.

    */
    /* Riordino le carte in mano in ordine crescente */
                 ...
                 int hand[][2])
{
    ...
    int risultato_Mano_1 = 10;
    int risultato_Mano_2 = 10;
